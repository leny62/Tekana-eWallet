...
import { AllowRoles, GetUser } from '../auth/decorators';
...
export class CustomerController {
  ...
  @ApiQuery({
    name: 'from',
    type: Number,
    description: 'walletId for your wallet',
  })
  @ApiQuery({
    name: 'to',
    type: Number,
    description: 'walletId for the wallet you want to transfer to',
  })
  @ApiQuery({
    name: 'toId',
    type: Number,
    description: 'userId for the user you want to send money',
  })
  @ApiForbiddenResponse({
    description: 'currency do notmatch or amount is less than one on account',
  })
  async createTransaction(
    ...
    @GetUser() user: User,
    @Query('toId', ParseIntPipe) toId: number,
  ) {
    return await this.customerService.transferMoney(
      to,
      from,
      dto,
      user.id,
  ...
  @ApiQuery({
    name: 'walletId',
    type: Number,
    description: 'walletId for the wallet your wallet',
  })
  ...
  @ApiForbiddenResponse({
    description: 'currency do notmatch or this not your  account',
  })
  @ApiBody({ type: topUpDto })
  async topUpWallet(
    @Query('walletId', ParseIntPipe) id: number,
    @Body() dto: topUpDto,
    @GetUser() user: User,
  ) {
